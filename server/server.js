...
app.get("/properties", async (req, res) => {
    try {
        ...
        res
            .status(500)
            .json({ error: "An error occurred while fetching property data" });
    }
});
...
app.post("/updateProperty/:hostname", async (req, res) => {
    ...
    try {
        ...
        res
            .status(500)
            .json({ error: "An error occurred while updating property data" });
    }
});
    ...
    try {
        ...
        uploadedFile.mv(uploadPath, async (err) => {
            ...
            try {
                ...
                const host = parsedData.hosts.find(
...
app.delete("/deleteProperty/:hostname", async (req, res) => {
    const hostname = req.params.hostname;
    ...
    try {
        ...
        // Find the index of the host by hostname
        const hostIndex = parsedData.hosts.findIndex(
            (host) => host.hostName === hostname
        );
        ...
        // Remove the host at the found index
        parsedData.hosts.splice(hostIndex, 1);
...
app.listen(5000, () => {
    console.log("Served started on port 5000");
});
